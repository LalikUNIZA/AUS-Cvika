...
namespace ds::amt {
    ...
    public:
        ...
        size_t calculateIndex(BlockType& block) const override;
    ...
    template<typename DataType>
    size_t ImplicitSequence<DataType>::calculateIndex(BlockType& block) const
    {
    ...
    {
        const size_t index = this->indexOfNext(this->calculateIndex(block));
        return index < this->size() ? &this->getMemoryManager()->getBlockAt(index) : nullptr;
    }
    ...
    {
        const size_t index = this->indexOfPrevious(this->calculateIndex(block));
